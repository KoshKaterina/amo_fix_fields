"""Авто-перенос дубля в ЗИН.

Когда менеджер ставит поле сделки «Причина отказа» (577623) = «Дубль сделки»
(enum 1041141), автоматически переводим сделку в «Закрыто и не реализовано» (143)
в ЕЁ ТЕКУЩЕЙ воронке (143 — системный статус, есть во всех воронках). Работает во
всех воронках. Вызов из вебхука /lead_change (должен быть быстрым) → работа в фоне.
...
Боевое (двигает реальные сделки) → за флагом DUP_AUTOCLOSE_ENABLED, по умолчанию ВЫКЛ.
"""
...
async def _maybe_close(lead_id) -> None:
    try:
        ...
        if _reason_enum_id(lead) != DUP_REASON_ENUM_ID:
            return  # причина не «Дубль сделки» (или уже сменили) — не трогаем
        ...
        logger.info(
            "Дубль→ЗИН: сделка %s переведена в 143 (воронка %s)", lead_id, pipeline_id
        )
    except Exception:
        logger.exception("Дубль→ЗИН: ошибка на сделке %s", lead_id)

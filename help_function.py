...
# Сервисные позиции заказа: не товар и не доставка. В «Корзину» им нельзя (клиент
# прочитает наценку как второй товар), в «Тип доставки» - тоже: 04.09.2026 менеджер
# получил там строку «CDEK: Самовывоз, 398.00 рублей Наценка за наложенный платеж,
# 196.00 рублей», стёр хвост руками, а синхронизация вернула его обратно. Такие
# позиции просто отбрасываем: сумма наложенного платежа берётся не отсюда, а из
# способа оплаты и суммы заказа (waybill_service.py, is_cod_payment).
SERVICE_PREFIXES = (
...
async def parse_the_cart_field(data: str):
    ...
    for item in items:
        ...
        if line.startswith(SERVICE_PREFIXES):
            continue  # сервисная позиция: ни в товары, ни в доставку

...
(заведён в МойСкладе 03.08). Правило простое и одностороннее:
    ...
    услуги нет, а склад стоит шоурумный                    → склад = Основной

Источник правды — УСЛУГА, а не склад: менеджер меняет способ доставки, склад
за ним идёт сам. Руками склад никто не трогает (прямое требование Кати).

⚠️ Возврат делаем ТОЛЬКО со шоурумного склада. Заказы на «Вскрытые», ЭРМС и
прочие склады живут по своим правилам — если услуги шоурума в них нет, это не
повод тащить их на Основной.
...
def target_store(order: dict) -> str | None:
    ...
    current = _store_id(order)
    if has_showroom_service(order):
        return None if current == MS_STORE_SHOWROOM_ID else MS_STORE_SHOWROOM_ID
    # услуги нет: возвращаем на Основной ТОЛЬКО шоурумные заказы
    if current == MS_STORE_SHOWROOM_ID:
        return MS_STORE_MAIN_ID
    return None
